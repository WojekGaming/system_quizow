...
<head>
    ...
    <title>Edytuj Quiz — Quizzies</title>
...
<div class="qb-app">
    ...
    <aside class="qb-sidebar">
        ...
        <div class="side-block">
            ...
            <div class="qb-field">
                ...
                <select id="quizCategory" class="qb-select">
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat->id }}" {{ old('category_id', $quiz->category_id) == $cat->id ? 'selected' : '' }}>
                            {{ $cat->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="qb-field">
                <label>Opis</label>
                <textarea id="quizDesc" class="qb-textarea" placeholder="Krótki opis quizu...">{{ old('description', $quiz->description) }}</textarea>
            </div>
            <div class="qb-toggle-row">
                <input type="checkbox" id="quizPremium" {{ $quiz->is_premium ? 'checked' : '' }}>
                <label for="quizPremium">Quiz Premium</label>
            </div>
            <div class="qb-toggle-row" style="margin-top:8px;">
                <input type="checkbox" id="quizActive" {{ $quiz->is_active ? 'checked' : '' }}>
...
<form id="submitForm" method="POST" action="{{ route('quizzes.update', $quiz) }}" style="display:none;">
    @csrf
    @method('PATCH')
