...
<head>
    ...
    <title>Wynik ÔÇö {{ $quiz->title }}</title>
...
<div class="dash-container" style="max-width:760px;">
    ...
    <div class="dash-panel" style="text-align:center;margin-bottom:1.5rem;">
        ...
        <div style="font-size:14px;color:rgba(255,255,255,0.4);">
            @if($percentage >= 70) ┼Üwietny wynik! ­čÄë
            @elseif($percentage >= 40) Nie┼║le, ale mo┼╝na lepiej ­čĺ¬
            @else Spr├│buj jeszcze raz ­čôÜ
            @endif
        </div>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:14px;margin-bottom:1.5rem;">
        ...
        ">
            <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:14px;">
                <span style="font-size:18px;flex-shrink:0;">{{ $result['is_correct'] ? 'Ôťů' : 'ÔŁî' }}</span>
            ...
            <div style="display:flex;flex-direction:column;gap:6px;padding-left:28px;">
                ...
                ">
                    ...
                    {{ $answer }}
                    @if($isCorrect) <span style="margin-left:auto;">Ôťô</span> @endif
                    @if($isSelected && !$isCorrect) <span style="margin-left:auto;">ÔťŚ</span> @endif
                </div>
                @endif
                @endforeach
            </div>
    ...
    <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;">
        <a href="{{ route('quiz.show', $quiz) }}" class="dash-cta-btn">­čöä Zagraj ponownie</a>
        <a href="{{ url('/') }}" style="
